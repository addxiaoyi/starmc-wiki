...
import { ChevronRight, Copy, Terminal, Check, ArrowUpRight, BookOpen, Wrench, History } from 'lucide-react';
...
const Home: React.FC = () => {
  ...
  const [theme] = useState<ThemeType>('blueprint');
  ...
  return (
    <div className={`min-h-screen transition-colors duration-700 ${theme === 'blueprint' ? 'bg-slate-50 dark:bg-slate-950' : theme === 'aura' ? 'bg-white dark:bg-slate-950' : 'bg-stone-50 dark:bg-slate-950'}`}>
      <div className="relative z-10 mx-auto max-w-8xl px-4 pt-8 sm:px-6 lg:px-8 lg:pt-12">
        <header className="grid grid-cols-1 gap-10 xl:grid-cols-12 xl:items-end xl:gap-14 mb-12 lg:mb-24">
          ...
          <div className="min-w-0 space-y-5 xl:col-span-4 xl:pt-6 sm:space-y-6 md:space-y-8">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:gap-4">
              <Link to="/wiki/index" onMouseEnter={() => import('./WikiPage')} className="inline-flex min-w-0 items-center justify-between rounded-xl bg-slate-950 px-6 py-4 font-semibold text-white transition-colors hover:bg-slate-800 sm:px-7 md:px-8 md:py-5">
                <span className="truncate">立即开始探索</span>
                <ChevronRight size={20} className="shrink-0" />
              </Link>
              <a href={OFFICIAL_WEBSITE} target="_blank" rel="noreferrer" className="inline-flex min-w-0 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-6 py-4 font-semibold text-slate-900 transition-colors hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-950 dark:text-white dark:hover:bg-slate-900 sm:px-7 md:px-8 md:py-5">
                官网
        ...
        <section className="grid grid-cols-1 gap-px overflow-hidden rounded-xl border border-slate-200 bg-white mb-20 md:mb-32 dark:border-slate-800 dark:bg-slate-950 sm:grid-cols-2 lg:grid-cols-3">
            {[
            { title: '生存与经营', desc: '围绕生存、农场和技能展开。', glyph: '生', radius: 'rounded-lg' },
            { title: '探索与任务', desc: '任务与资源点分布，适合跑图和收集。', glyph: '探', radius: 'rounded-xl' },
            { title: '社区互动', desc: '公会、婚姻和表情动作，让交流更自然。', glyph: '社', radius: 'rounded-2xl' },
          ].map((item, i) => (
            <article key={i} className="bg-white p-6 sm:p-8 md:p-10 dark:bg-slate-950">
              <div className="mb-5 flex items-center gap-4">
                <div className={`flex h-12 w-12 items-center justify-center ${item.radius} border border-slate-200 bg-white text-lg font-bold text-slate-800 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100`}>
                  {item.glyph}
                </div>
        ...
        <section className="mb-20 sm:mb-24 md:mb-40">
          ...
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-3 md:gap-8">
            {[
              ...
              <div key={i} className={`rounded-xl border bg-white p-6 dark:bg-slate-950 ${theme === 'blueprint' ? 'border-slate-200' : theme === 'aura' ? 'border-slate-200/70' : 'border-slate-200'}`}>
                <div className="mb-5 flex items-start justify-between gap-4">
                  <div className={`inline-flex items-center gap-2 rounded-lg border px-3 py-2 ${item.tone === 'cyan' ? 'border-cyan-200 text-cyan-700 dark:border-cyan-900/50 dark:text-cyan-300' : item.tone === 'sky' ? 'border-sky-200 text-sky-700 dark:border-sky-900/50 dark:text-sky-300' : 'border-indigo-200 text-indigo-700 dark:border-indigo-900/50 dark:text-indigo-300'}`}>
                    <span className="text-lg font-bold leading-none">{item.glyph}</span>
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-70">线路</span>
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className={`rounded-lg border px-4 py-3 ${theme === 'aura' ? 'bg-white/60 border-slate-200/70 dark:bg-slate-950/60 dark:border-slate-800' : 'bg-white border-slate-200 dark:bg-slate-950 dark:border-slate-800'}`}>
                  ...
                  <button
                    ...
                    className={`inline-flex w-full items-center justify-center gap-2 rounded-lg px-5 py-3 font-semibold transition-colors sm:w-auto ${copied === item.ip ? 'bg-emerald-500 text-white' : theme === 'blueprint' ? 'bg-slate-900 text-white hover:bg-slate-800 dark:bg-white dark:text-slate-900' : theme === 'aura' ? 'bg-blue-600 text-white hover:bg-blue-500' : 'bg-emerald-600 text-white hover:bg-emerald-500'}`}
                  >
        ...
        <footer className={`border-t pt-20 pb-24 space-y-16 ${theme === 'blueprint' ? 'border-slate-200 dark:border-slate-800' : theme === 'aura' ? 'border-blue-100 dark:border-blue-900/50' : 'border-emerald-100 dark:border-emerald-900/50'}`}>
          <section className="grid grid-cols-1 gap-8 lg:grid-cols-12">
            <div className="space-y-6 lg:col-span-5">
              <div className="flex items-center gap-3">
                ...
                <span className="text-2xl font-bold tracking-tighter uppercase dark:text-white">StarMC Wiki</span>
              </div>
            ...
            <div className="space-y-4 lg:col-span-3">
              <h4 className="text-sm font-bold uppercase tracking-widest text-slate-400">导航</h4>
            ...
            <div className="space-y-4 lg:col-span-4 xl:pl-2">
              <h4 className="text-sm font-bold uppercase tracking-widest text-slate-400">成员</h4>
              <a href="https://github.com/addxiaoyi" target="_blank" rel="noreferrer" className={`group flex items-center gap-4 rounded-xl border px-4 py-4 transition-colors hover:bg-slate-50 ${theme === 'blueprint' ? 'border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 dark:hover:bg-slate-900' : theme === 'aura' ? 'border-blue-100 bg-white/80 dark:border-blue-900/50 dark:bg-slate-950/80 dark:hover:bg-slate-900' : 'border-emerald-100 bg-white dark:border-emerald-900/50 dark:bg-slate-950 dark:hover:bg-slate-900'}`}>
                <img src="https://github.com/addxiaoyi.png" alt="addxiaoyi avatar" className="h-12 w-12 rounded-lg border border-slate-200 object-cover dark:border-slate-800" />
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-900 dark:text-white">addxiaoyi</span>
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest ${theme === 'blueprint' ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900' : theme === 'aura' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300'}`}>成员</span>
                  </div>
          ...
          <section className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {[
              ...
              <Link key={i} to={item.href} onMouseEnter={() => import('./WikiPage')} className={`group rounded-xl border p-6 transition-colors hover:bg-slate-50 ${theme === 'blueprint' ? 'border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 dark:hover:bg-slate-900' : theme === 'aura' ? 'border-blue-100 bg-white/80 dark:border-blue-900/50 dark:bg-slate-950/80 dark:hover:bg-slate-900' : 'border-emerald-100 bg-white dark:border-emerald-900/50 dark:bg-slate-950 dark:hover:bg-slate-900'}`}>
                <div className="mb-4 flex items-center gap-3">
                  <div className="rounded-lg bg-slate-100 p-3 text-slate-700 dark:bg-slate-900 dark:text-slate-300">
                    <item.icon size={20} />
                  </div>
