  ...
  return (
    <div className={`min-h-screen relative overflow-hidden transition-colors duration-700 ${
      ...
      <div className="relative z-10 mx-auto max-w-8xl px-4 pt-8 sm:px-6 lg:px-8 lg:pt-12">
        <header className="grid grid-cols-1 items-start gap-10 xl:grid-cols-12 xl:items-end xl:gap-14 mb-12 lg:mb-24">
          <div className="min-w-0 space-y-5 sm:space-y-6 xl:col-span-8 xl:space-y-8">
            <div className="inline-flex max-w-full items-center gap-2 rounded-full border border-slate-200/80 bg-white/85 px-3 py-1.5 text-slate-500 shadow-sm backdrop-blur-md animate-in fade-in duration-1000 dark:border-slate-800 dark:bg-slate-950/70 dark:text-slate-400">
            ...
            <h1 className="max-w-[10ch] text-[clamp(3rem,14vw,8rem)] font-semibold tracking-[-0.1em] leading-[0.84] text-slate-950 dark:text-white animate-in fade-in slide-in-from-left-8 duration-1000 delay-150 sm:max-w-none sm:text-[clamp(4rem,11vw,8rem)] xl:tracking-[-0.08em] xl:leading-[0.86]">
          ...
          <div className="min-w-0 space-y-5 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300 xl:col-span-4 xl:pt-6 sm:space-y-6 md:space-y-8">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:gap-4">
              <Link 
                ...
                className="inline-flex min-w-0 items-center justify-between rounded-2xl bg-slate-950 px-6 py-4 font-semibold text-white transition-all duration-300 hover:bg-slate-800 active:scale-[0.98] sm:px-7 md:px-8 md:py-5"
              >
        ...
        <section className="grid grid-cols-1 gap-px overflow-hidden rounded-[2rem] border border-slate-200 bg-white mb-20 md:mb-32 animate-in fade-in slide-in-from-bottom-12 duration-1000 delay-500 shadow-[0_20px_70px_rgba(15,23,42,.06)] dark:border-slate-800 dark:bg-slate-950 sm:grid-cols-2 lg:grid-cols-3">
          {[
            { 
              title: "生存与经营", 
              desc: "围绕生存、农场和技能展开。",
            ...
            { 
              title: "探索与任务", 
              desc: "任务与资源点分布，适合跑图和收集。",
              icon: <Compass className="text-sky-300" />, 
            },
            { 
              title: "社区互动", 
              desc: "公会、婚姻和表情动作，让交流更自然。",
              icon: <Globe className="text-indigo-300" />, 
            }
          ].map((item, i) => (
            <div key={i} className="relative overflow-hidden bg-white p-5 sm:p-7 md:p-10 transition-all duration-500 group hover:bg-slate-50 dark:bg-slate-950 dark:hover:bg-slate-900/70">
              ...
              <div className="relative z-10 min-w-0">
                <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl border border-slate-200 bg-white shadow-sm transition-transform duration-500 group-hover:scale-105 dark:border-slate-800 dark:bg-slate-950 sm:mb-6">
                  {item.icon}
                </div>
                <h3 className="mb-3 text-2xl font-semibold tracking-[-0.04em] text-slate-950 transition-transform group-hover:translate-x-0.5 dark:text-white sm:text-[1.85rem] md:text-3xl break-keep">{item.title}</h3>
                <p className="text-sm font-normal leading-relaxed text-slate-500 transition-transform delay-75 group-hover:translate-x-0.5 dark:text-slate-400 md:text-base break-words">{item.desc}</p>
              </div>
            </div>
          ))}
        </section>
        ...
        <section className="mb-20 sm:mb-24 md:mb-40">
          <div className="mb-8 flex flex-col gap-3 sm:gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex items-center gap-4 self-start lg:self-auto">
              <div className={`hidden h-px w-24 lg:block xl:w-40 ${
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
            {[
              { label: "Java版大厅主路", ip: SERVER_IPS.primary, icon: <Zap size={24} className={theme === 'voxel' ? 'text-emerald-500' : 'text-amber-500'} />, badge: "主线" },
              { label: "Java版大厅主力", ip: SERVER_IPS.secondary, icon: <Globe size={24} className={theme === 'voxel' ? 'text-teal-500' : 'text-blue-500'} />, badge: "主力" },
              { label: "Java版大厅备用", ip: SERVER_IPS.tertiary, icon: <Compass size={24} className={theme === 'voxel' ? 'text-amber-500' : 'text-emerald-500'} />, badge: "备用" }
            ].map((item, i) => (
              <div key={i} className={`group relative overflow-hidden rounded-[2rem] border p-5 sm:p-6 md:p-8 transition-all duration-500 ${
                theme === 'blueprint' ? 'bg-slate-100/60 border-slate-200 hover:border-slate-300 hover:bg-white dark:bg-slate-900/50 dark:border-slate-800' :
                theme === 'aura' ? 'bg-white/50 backdrop-blur-md border-white/50 hover:border-blue-300 hover:shadow-2xl hover:shadow-blue-500/10 dark:bg-slate-900/40 dark:border-slate-800/40' :
                'bg-stone-100/80 border-stone-200 hover:border-emerald-300 hover:bg-white dark:bg-slate-900/80 dark:border-slate-800'
              }`}>
                <div className={`absolute top-0 right-0 h-28 w-28 rounded-full -mr-14 -mt-14 blur-2xl transition-transform duration-700 group-hover:scale-150 ${
                ...
                <div className="relative z-10 mb-5 flex items-center justify-between gap-3 sm:mb-6">
                  <div className={`shrink-0 rounded-2xl p-3 shadow-sm ${theme === 'aura' ? 'bg-white/80 dark:bg-slate-800/80' : 'bg-white dark:bg-slate-800'}`}>
                    {item.icon}
                  </div>
                  <span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black text-center leading-4 sm:px-3 sm:text-xs ${
                ...
                <div className="relative z-10 space-y-4">
                  <div className="text-xs font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 sm:text-sm">{item.label}</div>
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                    <div className={`min-w-0 rounded-2xl border px-4 py-3 shadow-inner sm:px-5 sm:py-4 ${
                      ...
                      <code className="block truncate font-mono text-sm font-bold leading-6 text-slate-900 dark:text-white sm:text-base md:text-lg xl:text-xl">{item.ip}</code>
                    </div>
                    <button 
                      onClick={() => copyToClipboard(item.ip)}
                      className={`inline-flex w-full items-center justify-center gap-2 rounded-2xl px-5 py-3 font-black transition-all active:scale-95 sm:w-auto ${
                        copied === item.ip 
                          ? 'bg-emerald-500 text-white' 
                          : theme === 'blueprint' ? 'bg-slate-900 text-white hover:bg-slate-800 dark:bg-white dark:text-slate-900' :
                            theme === 'aura' ? 'bg-blue-600 text-white hover:bg-blue-500' :
        ...
        <footer className={`border-t pt-20 pb-24 space-y-16 ${
          ...
          <section className="grid grid-cols-1 gap-8 lg:grid-cols-12">
            ...
            <div className="space-y-4 lg:col-span-3">
              <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest">导航</h4>
